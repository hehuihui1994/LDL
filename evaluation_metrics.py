...
def squared_chi2(distribution_real, distribution_predict):
    ...
    temp = np.sum(numerator / denominator, 1)
    return np.sum(temp) / height
...
if __name__ == "__main__":
    ...
    print('euclidean : {}'.format(euclidean(real, predict)))
    
    print('sorensen : {}'.format(sorensen(real, predict)))
    print('squared_chi2 : {}'.format(squared_chi2(real, predict)))
    print('kl : {}'.format(kl(real, predict)))
    print('intersection : {}'.format(intersection(real, predict)))
    print('fidelity : {}'.format(fidelity(real, predict)))    